...
function update(group, activeAnchor) {
	...
	image.setSize(topRight.attrs.x - topLeft.attrs.x, bottomLeft.attrs.y
					- topLeft.attrs.y);
}
...
function initStage(images) {
	...
	$('#save').click(function() {
				stage.toDataURL(function(dataUrl) {
					...
					window.open(dataUrl);
						/*
						 * $.post("ajax.php", { data : dataUrl }, function(data) {
	...
	document.getElementById("preview").addEventListener("click", function() {
				stage.toDataURL(function(dataUrl) {
							/*
							 * here you can do anything you like with the data
							 * url. In this tutorial we'll just open the url
							 * with the browser so that you can see the result
							 * as an image
							 */
							window.open(dataUrl);
	...
	$("ul#img a").click(function() {
		...
		// addProduct($('img', this));
		addResizableProduct($('img', this));
			// addAnchor(this, 0, 0, "topLeft");
	...
	$('#addImg').on('click', function() {
				...
				// document[imageObj].src = src;
				// imageObj.attr("src", src);
				addResizableProduct(img)
			});

	function addResizableProduct(imgObj) {
		...
		var imageObj = new Image();
		// imageObj.src = imgObj.getAttribute('src');
		...
		imageObj.onload = function() {
			...
			addAnchor(imageGroup, 300, 0, "topRight");
			addAnchor(imageGroup, 300, 320, "bottomRight");
			addAnchor(imageGroup, 0, 320, "bottomLeft");
			// stage.add(layer);
			imageGroup.add(imageImg);

			imageImg.on("mouseover", function() {
						...
						writeMessage(messageLayer,
								"Drag Image Into Position Double Click To Remove");
					});
			imageImg.on("mouseout", function() {
						var imagelayer = this.getLayer();
						document.body.style.cursor = "default";
			...
			imageImg.on("dblclick dbltap", function() {
				console.log(imageImg)
				layer.remove(imageGroup)
				layer.clear();
				layer.draw();
				// layer.draw();
				console.log(stage)
					// stage.remove()
					/*
					 * layer.remove(imageImg); layer.clear(); layer.draw();
					 * stage.draw();
					 */
				});
			// S layer.add(imageImg);
	...
	function addText() {
		...
		rect.on("mouseover dragmove", function() {
					...
					shapesLayer.draw();
					writeMessage(messageLayer,
							"Double Click To Remove Or Edit Text");
				})
		...
		complexText.on("mouseover dragmove", function() {
					...
					shapesLayer.draw();
					writeMessage(messageLayer,
							"Drag Corners Increse Or Decrease Text Size");
				})
	...
	darthVaderImg.on("mouseover", function() {
		...
		layer.draw();
		writeMessage(messageLayer,
				"Drag Image Into Position Click And Drag The Corners To Re-size");
	...
	// yoda
	/*
	 * var yodaImg = new Kinetic.Image({ x : 0, y : 0, image : images.yoda,
	 * width : 691, height : 450, name : "image" });
	 * 
...
window.onload = function() {
	var sources = {
		...
		yoda : "images/Valentines-Day.jpg"
	};
...
$(window).load(function(images) {
			...
			$("#background").click(function() {
				...
				stage.setFill('blue');
					/*
					 * var yodaImg = new Kinetic.Image({ x : 0, y : 0, image :
					 * images.yoda, width : 691, height : 450, name : "image"
					 * }); var stage2 = new Kinetic.Stage({ container :
					 * "container2", width : 130, height : 40 })
					 * yodaGroup.add(yodaImg);
					 * 
					 * console.log(this)
					 */
